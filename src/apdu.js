 ...
 * @constructor
 * @param {Buffer} command the command APDU
	...
	if (arguments.length > 0) {
		...
		if (arg instanceof ArrayBuffer) {
			if (arguments.length != 1) {
				throw new Error("APDU", APDU.SW_GENERALERROR, "Only one argument of type ByteString expected");
			}
			this.fromBuffer(arg);
		} else {
			if ((arguments.length < 4) || (arguments.length > 6)) {
				throw new Error("APDU", APDU.SW_GENERALERROR, "4 to 6 arguments expected");
			}
			
			for (var i = 0; i < 4; i++) {
				if (typeof(arguments[i]) != "number") {
					throw new Error("APDU", APDU.SW_GENERALERROR, "Argument must be of type Number");
				}
			}
			...
			if (arguments.length > i) {
				if (arguments[i] instanceof ArrayBuffer) {
 ...
 * @param {Buffer} buffer
 */
APDU.prototype.fromBuffer = function(buffer) {
  dv = new DataView(buffer);
  if (dv.byteLength < 4) {
    throw new Error("APDU", APDU.SW_GENERALERROR, "Command APDU must be at least 4 bytes long");
  }
  this.cla = dv.getUint8(0);
  this.ins = dv.getUint8(1);
  this.p1 = dv.getUint8(2);
  this.p2 = dv.getUint8(3);
  
  if (dv.byteLength > 4) {
    var extended = false;
    
    var i = 4;
    var l = dv.byteLength - i;
    var n = dv.getUint8(i++);
    l--;
    ...
    if ((n == 0) && (l > 0)) {
        extended = true;
        if (l < 2) {
          throw new Error("APDU",  APDU.SW_WRONGLENGTH, "Extended length APDU too short");
        }
        n = (dv.getUint8(i) << 8) + dv.getUint8(i + 1);
        i += 2;
        l -= 2;
    }
    ...
    if (l > 0) {	// Case 3s / Case 3e / Case 4s / Case 4e
        if (l < n) {
          throw new Error("APDU", APDU.SW_WRONGLENGTH, "Invalid Lc in APDU");
        }
        this.cdata = dv.buffer.slice(i, n);
        i += n;
        l -= n;
        ...
        if (l > 0) {	// Case 4s / Case 4e
          ...
          if (extended) {
              if (l < 1) {
                throw new Error("APDU", APDU.SW_WRONGLENGTH, "Invalid Le in extended APDU");
              }
              n = (n << 8) + dv.getUint8(i++);
              l--;
          }
    ...
    if (l > 0) {
        throw new Error("APDU", APDU.SW_WRONGLENGTH, "Too many bytes in APDU");
    }
  }
}


APDU.prototype.isExtended = function() {
  var hasCData = (typeof(this.cdata) != "undefined");
  var hasNe = (typeof(this.ne) != "undefined");
  return ((hasCData && this.cdata.byteLength > 255) || 
              (hasNe && this.ne > 256));
}
...
APDU.prototype.getCommandAPDU = function() {
  var hasCData = (typeof(this.cdata) != "undefined");
  var hasNe = (typeof(this.ne) != "undefined");
  var extended = ((hasCData && this.cdata.byteLength > 255) || 
              (hasNe && this.ne > 256));
  var size = 0;
  size += 4;
  if (extended) {
  ...
  var i = 0;
  dv.setUint8(i++, this.cla);
  dv.setUint8(i++, this.ins);
  dv.setUint8(i++, this.p1);
  dv.setUint8(i++, this.p2);

  if (extended) {
    dv.setUint8(i++, 0);
  }

  if (hasCData && this.cdata.byteLength > 0) {
    if (extended) {
      dv.setUint8(i++, this.cdata.byteLength >> 8);
    }
    dv.setUint8(i++, this.cdata.byteLength & 0xFF);
    var cdv = new DataView(this.cdata);
  ...
  if (hasNe) {
    if (extended) {
      dv.setUint8(i++, this.ne >> 8);
    }
    dv.setUint8(i++, this.ne & 0xFF);
  }
...
APDU.prototype.decodeResult = function(data) {
  var dv = new DataView(data);
  if (dv.byteLength === 2) {
    this.SW = dv.getUint8(0) << 8;
    this.SW ^= dv.getUint8(1);
  } else if (dv.byteLength > 2) {
    this.rdata = data.slice(0, data.byteLength-2);
    this.SW = dv.getUint8(data.byteLength-2) << 8;
    this.SW ^= dv.getUint8(data.byteLength-1);
  }
}
 ...
 *
 * @param {ArrayBuffer} cdata the command data
 */
...
APDU.prototype.getCDataAsTLVList = function() {
	if (typeof(this.cdata) == "undefined") {
		throw new Error("APDU", APDU.SW_INVDATA, "No data in command APDU");
	}
	...
	try	{
		...
		throw new Error("APDU", APDU.SW_INVDATA, "Invalid TLV data in command APDU");
	}
 ...
 * @return the status word
 */
APDU.prototype.getSW1 = function() {
	return (this.SW >> 8);
}
 ...
 * @return the status word
 */
APDU.prototype.getSW2 = function() {
	return (this.SW & 0xFF);
}
